...
"""
    ...
    def test_sim_dds_uses_clean_default_domain_and_runtime_gates(self):
        bringup = (ROOT / "scripts/nav/bringup.sh").read_text()
        ...
        self.assertNotIn('ROS_LOCALHOST_ONLY', bringup)
        self.assertNotIn('ROS_LOCALHOST_ONLY', supervisor)
    ...
    def test_local_planner_stops_inside_manipulation_work_pose_tolerance(self):
        ...
        self.assertIn('validate_nav_profile.py', bringup)
        self.assertNotIn('ros2 param set /localPlanner goalReachedThreshold', supervisor)
        self.assertIn(
            'LOCAL_PLANNER_GOAL_REACHED_THRESHOLD:-0.15',
            supervisor,
        )
