...
## 4. Monitoring mid-run (what "healthy" looks like)
...
## 5. Acceptance (after training completes — run at a clean sim window)
...
Pass criteria (per segment, in the JSON `pass` field):
| # | Segment | Criterion |
|---|---------|-----------|
| 1 | zero-cmd 30 s | GT displacement speed **< 0.02 m/s** and no fall (baseline crept 0.075) |
| 2 | low-cmd ladder 0.15/0.30/0.60, 10 s each | `|achieved-cmd|/cmd ≤ 0.35` and no fall (vs old 0.257/0.383/0.644 over-shoot) |
| 3 | wz=±1.4 step ×5 | **fall rate == 0** (fall = |roll|>60° or |pitch|>60°) |
| 4 | arc (0.3, 0.5), 10 s | no fall and `|speed-0.3|/0.3 ≤ 0.35` |
...
## 6. Plan-a escalation (only if plan-d acceptance fails)
...
CoM front/up-biased). Accept the plan-a ckpt with the same §5 battery.

## 7. Deployment switch (after acceptance passes — this is the go-live)
