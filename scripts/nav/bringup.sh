...
_local_planner_contract() {
  docker exec navstack bash -c '
    source /opt/ros/jazzy/setup.bash
    source /ws/install/setup.bash
    actual="$(ros2 param get /localPlanner goalReachedThreshold 2>/dev/null \
      | sed -n "s/^Double value is: //p")"
    python3 - "$actual" "$LOCAL_PLANNER_GOAL_REACHED_THRESHOLD" <<"PY"
import math
import sys
...
if not (math.isfinite(actual) and math.isfinite(expected)):
    raise SystemExit(1)
if expected <= 0.0 or expected >= 0.20 or abs(actual - expected) > 1e-9:
    raise SystemExit(1)
PY
