...
> 完整的新机（台式机 sim）从零拉起序列——每步预期耗时 + 验证点 + 导航栈镜像构建
> 5 步——见 **[docs/user-manual.md 第 1 章](docs/user-manual.md)**。下面是速览：

```bash
git clone https://github.com/Z-Robotics-Lab/go2W_Sim.git && cd go2W_Sim
bash scripts/clone_deps.sh          # IsaacLab v2.3.2 / robot_lab v2.3.2 / go2w URDF（含重训补丁自动 apply）
...
docker login nvcr.io && docker pull nvcr.io/nvidia/isaac-sim:5.1.0   # NGC 基础镜像（需登录）
...
# 导航栈镜像 navstack:ready（clone fork→patch→jazzy-dev→colcon→commit）：见 user-manual.md §1.5
```

> RL 策略权重已入库 `assets/policies/`（clone 即有，`bringup.sh` 默认指它，无需训练即可拉起）。
...
# 2) 导航栈容器（首次搭建见 docs/user-manual.md §1.5：clone
#    github.com/Yuxin916/Navigation-Physical-Experiment (jazzy 分支) 到 refs/ →
#    patch_navstack.sh → 建 jazzy-dev 镜像 → colcon build → commit navstack:ready）
