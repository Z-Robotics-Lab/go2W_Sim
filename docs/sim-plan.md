...
| 世界/传感器 frame | `map` / `sensor`（ARISE 重力水平化虚拟帧）；原始测量为 `mid360_raw` | livox_mid360.yaml + 仿真桥契约 |
| IMU-laser 外参 | R=I，t=[-0.011,-0.02329,0.04412]（= Mid-360 出厂内置 IMU 偏移）| livox/livox_mid360_calibration.yaml |
| 倾斜安装支持 | 原始 LiDAR/IMU 共享物理 +20° 测量帧；ARISE 用同一重力旋转水平化两路数据，因此相对 `imu_laser_rotation_offset` 必须为零 | 2026-07-13 frame 审计 |
| 传感器安装参数 | `sensorOffsetX/Y/Z` → Go2W 用 0.27/0/0.10；`sensorMountPitch` 默认 +0.349066rad | system_isaac_sim launch reference |
...
1. Isaac 侧（M2 扩展）：RTX Lidar（Mid-360 近似配置）挂 `mid360_link`，以物理
   `mid360_raw` frame 发布 PointCloud2；IMU sensor 挂 Mid-360 内置 IMU 位置并保持同一
   斜装测量语义 → /imu/data 200Hz；桥不得再手工旋转任一路数据。
...
**载荷轮预注册判据（2026-07-07 CEO 直接指示"训练把手臂和负重考虑进去"；开火前冻结）**：
配方：plan-a（Payload-v0：base add 5-8kg 中心化 + CoM 前偏 x(0.03,0.10)/上偏 z(0.03,0.10)），
继承 v2 配方（rel_standing 0.12/税 -0.005），从 model_3497 resume +1000 iter；nuc_weight
已修 1.8kg（部署载荷 6.46kg 与训练中心/实物对齐）。门：
- ① 带载部署面漂移 ≤0.02（不得从 0.0101 显著回退）
- **⑤ 主判据（新增）：带载 pitch_var ≤1.5× 裸机**（现状 2.6×，A/B 判决遗留）
- ③ 零摔，且爆发下瞬态侧倾违规显著缩（对照 r3 E0''：50/1200、最深 -0.811）
- ④ 0.3/0.6 跟踪 rel_err ≤0.10/0.05（微调松弛但不许崩）
- 原生 E-T 随机体群中位报数（期望从 0.0343 收敛，不设门）
过 → 切默认 + E0'' 产品脸 + 提交；不过 → 单变量调整至多一轮重试，再不过停手全量报数。
**裁定（2026-07-07 执行后回填）**：①③④过（model_5495：0.0112/零摔/0.0603+0.0367；
E-T 随机体群 0.0157 收敛过 0.02），**⑤ 比值门两候选 FAIL（2.43×/4.23×）→ 未落地**。
但绝对值两身体塌一个量级（带载 pitch std 0.76°→0.27°）——比值形式在噪声级不可达，
建议改绝对门（≤5e-5 rad²，两候选 2× 裕度过），待 CEO 裁定后 model_5495 可即落地。
现任 model_3497 在修复后 6.46kg 体上重锚：①0.0049/④0.0337+0.0063 更好，产品基线无恙。
**⑤ 门形修正（编排者代 CEO 裁定 2026-07-07，可复议）**：比值门（≤1.5×）判定为门形
缺陷——两侧绝对值塌至噪声级后比值无物理意义（分母同步收敛；前偏质量 pitch 响应更大是
恒真项）。原比值判据与 2.43×/4.23× 的 FAIL 记录**原样保留于上**（不删不改）。修正后门形：
**带载行驶段 pitch_var ≤ 5e-5 rad²（绝对门）**——原始意图"带载稳定性收敛到健康"的绝对
量化重述。两候选裕度：model_4496 = 1.825e-5（2.7×）、model_5495 = 2.325e-5（2.2×），均过。
据此落地 model_5495（全门通过：①0.0112 ③零摔 ④0.0603/0.0367 ⑤abs 2.3e-5）。
