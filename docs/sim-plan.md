...
| 倾斜安装支持 | 20° 只保留在 URDF/USD 物理挂载；当前 Isaac ROS 输出已是导航对齐系，`imu_laser_rotation_offset` 必须为零 | 2026-07-10 在线平面拟合 |
...
1. Isaac 侧（M2 扩展）：RTX Lidar（Mid-360 近似配置）挂 `mid360_link` → PointCloud2；
   IMU sensor 挂 mid360 内置 IMU 位置（对齐出厂位置；不重复应用视觉挂载角）→ /imu/data 200Hz；
