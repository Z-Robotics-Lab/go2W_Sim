...
CMU 差速/麦轮自主导航栈（SLAM + 地形分析 + FAR/local planner + TARE 探索）。
`refs/` 是 gitignored 的（不随本仓库分发），故需手动 clone 到 `refs/` 后打补丁、建镜像、编译。
基础镜像 `osrf/ros:jazzy-desktop` 是**公开**的（无需 NGC 登录，与 Isaac 侧不同）。

> **导航栈的来源与我们的本地定制（复刻本机配置的关键，务必读懂）**
> 上游血缘：`jizhang-cmu`（CMU 原始栈）→ `Yuxin916/Navigation-Physical-Experiment` @ `jazzy`
> （**个人 fork**，非 Z-Robotics-Lab org 仓——这是一条外部依赖，作者改分支/删库会断源）→
> 我们的本地定制。一份**全新 clone 是纯上游**，会丢掉我们全部定制；下面 5 步用**三个机制**把
> 定制还原回来，缺一步孪生就退化（点云歪、坐标系斜、探索不自启）：
> - **机制 A** `patch_navstack.sh`（sed/python 变换，每次跑重新推导）：`use_sim_time` 全链切真、
>   生成 `system_isaac_sim*.launch.py`、`kAutoStart:false`。
> - **机制 B** `sync_navstack_files.sh`（真源在 `scripts/nav/`，拷进 refs）：SLAM 外参
>   `livox_mid360{,_calibration}.yaml`（**含 `sensor_mount_pitch_deg:20` + 20° IMU↔laser**）、
>   `agent_bridge.py`/`pc2_to_livox.py`/`run_navstack.sh` 等。
> - **机制 C** `navstack_patch/apply.sh`（refs-only 直改，静态 patch）：`imuPreintegration.cpp`
>   去斜 C++（patch 01）、`obstacleHeightThre 0.20`（patch 02, fix-c）、SLAM 节点 respawn（patch 03）。
>
> A 与 C 都由第 2 步的 `patch_navstack.sh` **一条命令自动完成**（它内部依次调 B、C）。
> 每个定制文件**恰好归属一个机制**、互斥无重叠——权威归属表见
> `scripts/nav/navstack_patch/manifest.md`（含"故意排除"的 fix-a WIP 说明）。
...
# 4) 在容器里 colcon 编译工作区。arise_slam_mid360(+msgs)/livox_driver2 三包在此**故意跳过**
#    （它们的构建约束特殊，与主工作区分开，见 refs/.../docker/README.md）——下一步 4b 单独编 SLAM。
...
# 4b) 单独编 arise_slam_mid360（**强制，别跳**）。两个理由：
#     ① 第 4 步 --packages-skip 了它，jazzy-dev 基础镜像也没预编——不编则 install/arise_slam_mid360
#        不存在，SLAM 核心缺包起不来；
#     ② 第 2 步的 patch 01 改的正是它的 C++（sensor_mount_pitch_deg 去斜）——不编则点云/里程计
#        仍带 20° 倾角（就是"rviz 点云是歪的"那个病）。~62s（DEBUG_extrinsic_friction.md 实测）。
docker run --rm --memory 8g -v "$PWD/refs/Navigation-Physical-Experiment":/ws -w /ws \
    jazzy-dev:latest bash -lc \
    'source /opt/ros/jazzy/setup.bash && \
     colcon build --packages-select arise_slam_mid360 arise_slam_mid360_msgs'

# 5) 固化成 navstack:ready（bringup.sh/restart_all.sh 用这个 tag 起容器）。
#    注意：navstack:ready 必须在 4+4b 都编完后再 commit，否则镜像里没有去斜过的 arise_slam。
...
ls refs/Navigation-Physical-Experiment/install/arise_slam_mid360   # 存在=去斜 SLAM 已编
```
...
>
> `navstack:ready` 是「jazzy-dev + 已编译工作区（含 4b 的去斜 arise_slam）」的 `docker commit`
> 快照——目前无一键脚本（承诺中的 `scripts/nav/setup_navstack.sh` 尚未落地）；以上 **1→2→3→4→4b→5**
> 六步就是完整可复现序列。复刻校验：起栈后看 rviz 点云不再 20° 上仰（去斜 C++ 已编=4b 生效）、
> `scripts/nav/status.sh` 全绿。若点云仍歪，几乎必然是漏了 4b（arise_slam 用了未打补丁的旧二进制）。
